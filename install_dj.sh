...
# Define the PowerShell profile path
...
function dj {
    param(
        [string]$package,
        [string]$extraArg = $null # Optional extra argument for commands like "startapp"
    )

    # Check if a query name is provided
    if (-not $package) {
...
# Reload the PowerShell profile by invoking PowerShell from within Bash
